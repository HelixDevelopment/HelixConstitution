#!/usr/bin/env bash
# cm_covenant_114_199_propagation.sh — CM-COVENANT-114-199-PROPAGATION gate (exact-reproduction-sequence mandate (when a working reproduction exists, the investigation MUST use ITS exact sequence)).
#
# ── Purpose ──────────────────────────────────────────────────────────────────
# §11.4.199 (exact-reproduction-sequence mandate (when a working reproduction exists, the investigation MUST use ITS exact sequence)) requires
# the anchor literal `11.4.199` to be present in every owned governance
...
#
# ── Usage ────────────────────────────────────────────────────────────────────
#   cm_covenant_114_199_propagation.sh [--root <consumer-root>] [--quiet]
...
#
# ── Cross-references ──────────────────────────────────────────────────────────
#   §11.4.199 (exact-reproduction-sequence mandate), §11.4.157
#   (five-carrier lockstep), §11.4.35 (inheritance), §11.4.28 (decoupling),
#   §11.4.102 / §11.4.146 / §11.4.115 (the reproduce-first anchors §11.4.199 strengthens — they mandate THAT you reproduce, §11.4.199 mandates HOW), §1.1 (paired mutation — strip
...
#
# ── Exit codes ───────────────────────────────────────────────────────────────
#   0 — every discovered owned carrier carries the `11.4.199` literal.
...
ANCHOR="11.4.199"
GATE="CM-COVENANT-114-199-PROPAGATION"
...
if [ "$fail" -gt 0 ]; then
    echo "❌ ${GATE}: FAIL — ${fail} owned carrier(s) missing §11.4.199 anchor"
    exit 1
fi
echo "✅ ${GATE}: PASS — every owned carrier carries §11.4.199"
exit 0
