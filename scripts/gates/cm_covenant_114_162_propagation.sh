#!/usr/bin/env bash
# cm_covenant_114_162_propagation.sh — CM-COVENANT-114-162-PROPAGATION gate.
#
# ── Purpose ──────────────────────────────────────────────────────────────────
# §11.4.162 (OpenDesign UI design system mandate) requires the anchor literal
# `11.4.162` to be present in every owned governance context-carrier file
# (CLAUDE.md / AGENTS.md / QWEN.md / GEMINI.md) across the consumer fleet, per
# the §11.4.157 five-carrier-lockstep + §11.4.35 inheritance rules. This gate
# scans the fleet, reports which carriers carry the literal (PRESENT) and which
# are MISSING it, and exits nonzero if any discovered carrier omits it.
#
# Mirrors the house propagation-gate concept (literal-anchor presence across the
# consumer fleet) used by every other CM-COVENANT-114-NNN-PROPAGATION gate.
#
# ── Usage ────────────────────────────────────────────────────────────────────
#   cm_covenant_114_162_propagation.sh [--root <consumer-root>] [--quiet]
...
#
# ── Cross-references ──────────────────────────────────────────────────────────
#   §11.4.162 (OpenDesign mandate), §11.4.157 (five-carrier lockstep),
#   §11.4.35 (inheritance), §11.4.28 (decoupling), §1.1 (paired mutation —
#   strip the literal from a carrier → this gate FAILs).
#
# ── Exit codes ───────────────────────────────────────────────────────────────
#   0 — every discovered owned carrier carries the `11.4.162` literal.
...
if [ "$fail" -gt 0 ]; then
    echo "❌ ${GATE}: FAIL — ${fail} owned carrier(s) missing §11.4.162 anchor"
    exit 1
fi
echo "✅ ${GATE}: PASS — every owned carrier carries §11.4.162"
exit 0
