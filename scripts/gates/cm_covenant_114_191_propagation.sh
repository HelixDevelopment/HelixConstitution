#!/usr/bin/env bash
# cm_covenant_114_191_propagation.sh — CM-COVENANT-114-191-PROPAGATION gate.
#
# ── Purpose ──────────────────────────────────────────────────────────────────
# §11.4.191 (Work-to-track/branch binding enforcement — a logic-group's work
# can NEVER be committed OR dispatched onto the wrong track/branch) requires
# the anchor literal `11.4.191` to be present in every owned governance
...
#
# ── Usage ────────────────────────────────────────────────────────────────────
#   cm_covenant_114_191_propagation.sh [--root <consumer-root>] [--quiet]
...
#
# ── Cross-references ──────────────────────────────────────────────────────────
#   §11.4.191 (work-to-track/branch binding enforcement), §11.4.157
#   (five-carrier lockstep), §11.4.35 (inheritance), §11.4.28 (decoupling),
#   §11.4.181 / §11.4.182 (the branch-NAME consistency anchors §11.4.191
...
#
# ── Exit codes ───────────────────────────────────────────────────────────────
#   0 — every discovered owned carrier carries the `11.4.191` literal.
...
ANCHOR="11.4.191"
GATE="CM-COVENANT-114-191-PROPAGATION"
...
if [ "$fail" -gt 0 ]; then
    echo "❌ ${GATE}: FAIL — ${fail} owned carrier(s) missing §11.4.191 anchor"
    exit 1
fi
echo "✅ ${GATE}: PASS — every owned carrier carries §11.4.191"
exit 0
