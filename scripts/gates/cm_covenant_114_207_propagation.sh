#!/usr/bin/env bash
# cm_covenant_114_207_propagation.sh — CM-COVENANT-114-207-PROPAGATION gate (instant multi-stream resume engine — continuum).
#
# ── Purpose ──────────────────────────────────────────────────────────────────
# §11.4.207 (instant multi-stream resume engine: whole-fleet continuation
# state is a durable, content-addressed, atomically-committed snapshot
# resumed in O(changed) reads, not an O(history) re-read) requires the anchor
# literal `11.4.207` to be present in every owned governance context-carrier
...
# gate — modeled verbatim on cm_covenant_114_201_propagation.sh (only the
# ANCHOR/GATE constants + this header differ).
#
# ── Usage ────────────────────────────────────────────────────────────────────
#   cm_covenant_114_207_propagation.sh [--root <consumer-root>] [--quiet]
...
#
# ── Cross-references ──────────────────────────────────────────────────────────
#   §11.4.207 (instant multi-stream resume engine mandate), §11.4.157
#   (five-carrier lockstep), §11.4.35 (inheritance), §11.4.28(C) (depth-1
#   reusable-engine submodule carve-out this anchor uses), §1.1 (paired
#   mutation — strip the literal from a carrier → this gate FAILs). Note
#   (§11.4.6 honest boundary): this gate checks the four MIRROR carriers
...
#   as cm_covenant_114_201_propagation.sh).
#
# ── Exit codes ───────────────────────────────────────────────────────────────
#   0 — every discovered owned carrier carries the `11.4.207` literal.
...
ANCHOR="11.4.207"
GATE="CM-COVENANT-114-207-PROPAGATION"
...
if [ "$fail" -gt 0 ]; then
    echo "❌ ${GATE}: FAIL — ${fail} owned carrier(s) missing §11.4.207 anchor"
    exit 1
fi
echo "✅ ${GATE}: PASS — every owned carrier carries §11.4.207"
exit 0
