#!/usr/bin/env bash
# cm_covenant_114_187_propagation.sh — CM-COVENANT-114-187-PROPAGATION gate.
#
# ── Purpose ──────────────────────────────────────────────────────────────────
# §11.4.187 (Automatic multi-track ruler orchestration) requires the anchor
# literal `11.4.187` to be present in every owned governance context-carrier
...
# gate — modeled verbatim on cm_covenant_114_162_propagation.sh (only the
# ANCHOR/GATE constants + this header differ).
#
# ── Usage ────────────────────────────────────────────────────────────────────
#   cm_covenant_114_187_propagation.sh [--root <consumer-root>] [--quiet]
...
#
# ── Cross-references ──────────────────────────────────────────────────────────
#   §11.4.187 (automatic multi-track ruler orchestration), §11.4.157
#   (five-carrier lockstep), §11.4.35 (inheritance), §11.4.28 (decoupling),
#   §1.1 (paired mutation — strip the literal from a carrier → this gate
#   FAILs). Note (§11.4.6 honest boundary): this gate checks the four MIRROR
#   carriers (CLAUDE/AGENTS/QWEN/GEMINI.md) — Constitution.md itself is the
#   anchor's own source-of-truth location and is guaranteed to carry the
#   literal by construction, so it is intentionally not re-scanned here (same
#   convention as cm_covenant_114_162_propagation.sh). A consuming project
#   that also wants Constitution.md itself checked (e.g. as part of a fixed
#   9-file governance-fleet list) wires that as an additional, project-
#   specific inline check — see qa-results/multitrack/rb10_snippets/gate.sh
#   for the ATMOSphere-specific 9-file instantiation of this same literal.
#
# ── Exit codes ───────────────────────────────────────────────────────────────
#   0 — every discovered owned carrier carries the `11.4.187` literal.
...
if [ "$fail" -gt 0 ]; then
    echo "❌ ${GATE}: FAIL — ${fail} owned carrier(s) missing §11.4.187 anchor"
    exit 1
fi
echo "✅ ${GATE}: PASS — every owned carrier carries §11.4.187"
exit 0
