#!/usr/bin/env bash
# cm_covenant_114_196_propagation.sh — CM-COVENANT-114-196-PROPAGATION gate (native-first alias priority + real-signal limit/subscription tracking + auto-rebind-on-recovery + RB-02 resource-detection-by-real-identity guard fix).
#
# ── Purpose ──────────────────────────────────────────────────────────────────
# §11.4.196 (Native-alias-first priority + per-alias real-signal limit/subscription
# tracking + auto-rebind-on-recovery + RB-02 guard fix) requires
# the anchor literal `11.4.196` to be present in every owned governance
...
#
# ── Usage ────────────────────────────────────────────────────────────────────
#   cm_covenant_114_196_propagation.sh [--root <consumer-root>] [--quiet]
...
#
# ── Cross-references ──────────────────────────────────────────────────────────
#   §11.4.196 (work-to-track/branch binding enforcement), §11.4.157
#   (five-carrier lockstep), §11.4.35 (inheritance), §11.4.28 (decoupling),
#   §11.4.181 / §11.4.182 (the branch-NAME consistency anchors §11.4.196
#   generalises to work-PLACEMENT consistency), §1.1 (paired mutation — strip
...
#
# ── Exit codes ───────────────────────────────────────────────────────────────
#   0 — every discovered owned carrier carries the `11.4.196` literal.
...
if [ "$fail" -gt 0 ]; then
    echo "❌ ${GATE}: FAIL — ${fail} owned carrier(s) missing §11.4.196 anchor"
    exit 1
fi
echo "✅ ${GATE}: PASS — every owned carrier carries §11.4.196"
exit 0
