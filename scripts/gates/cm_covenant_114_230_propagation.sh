#!/usr/bin/env bash
# cm_covenant_114_230_propagation.sh — CM-COVENANT-114-230-PROPAGATION gate
# (parallelized-pipeline methodology: build↔validate overlap + affected-
# stages-only re-run + parallel multi-device testing/recording +
# fan-out-subagents-always).
#
# ── Purpose ──────────────────────────────────────────────────────────────────
# §11.4.230 requires the anchor BLOCK (the anchor's own governance paragraph,
# not merely the bare literal `11.4.230` — §11.4.227(B)) to be present
...
#     the anchor (e.g. "... per §11.4.230(A) ...") is a CARRIER and MUST NOT
...
#                              for §11.4.185 and later anchors, incl. 230-233)
...
# so a numeric prefix-match (e.g. "2301") can never be mistaken for "230".
#
...
#
# ── Usage ────────────────────────────────────────────────────────────────────
#   cm_covenant_114_230_propagation.sh [--root <consumer-root>] [--quiet]
...
#
# ── Cross-references ──────────────────────────────────────────────────────────
#   §11.4.230 (parallelized-pipeline methodology), §11.4.227(B) (anchor-block
...
#
# ── Exit codes ───────────────────────────────────────────────────────────────
#   0 — every discovered owned carrier carries exactly ONE `11.4.230` block,
...
ANCHOR="11.4.230"
GATE="CM-COVENANT-114-230-PROPAGATION"
...
# numeric prefix-match (e.g. never confuse "2301" with "230").
