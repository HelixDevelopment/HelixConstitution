#!/usr/bin/env bash
# cm_covenant_114_162_propagation_mutation_test.sh — §1.1 paired-mutation
# meta-test for CM-COVENANT-114-162-PROPAGATION (§11.4.162).
#
# ── Purpose ──────────────────────────────────────────────────────────────────
# Proves CM-COVENANT-114-162-PROPAGATION is NOT a bluff gate: builds two
# disposable fleet fixtures under a temp dir — one with a carrier MISSING the
# `11.4.162` literal (MUST FAIL) and one where every carrier carries it (MUST
...
#
# Modeled on PART 1 of cm_opendesign_ui_system_mutation_test.sh, which already
# exercises this exact gate under a differently-named file. This sibling gives
# that pattern the canonical name covenant_propagation_suite.sh's `mutations`
# mode requires (cm_covenant_114_<slug>_propagation_mutation_test.sh) so the
# 162 row can be added to covenant_propagation_anchors.tsv without the suite
# exiting rc=127 looking for a file that did not exist under this name.
#
# ── Usage ────────────────────────────────────────────────────────────────────
...
#
# ── Dependencies ─────────────────────────────────────────────────────────────
#   bash, the sibling cm_covenant_114_162_propagation.sh gate script. Parses
#   clean under bash -n.
#
# ── Cross-references ──────────────────────────────────────────────────────────
#   §1.1 (paired mutation), §11.4.162, §11.4.157 (five-carrier lockstep),
...
GATE="${SCRIPT_DIR}/cm_covenant_114_162_propagation.sh"

[ -f "$GATE" ] || { echo "META: gate script missing: $GATE" >&2; exit 2; }

TMP="$(mktemp -d "${TMPDIR:-/tmp}/cm162_mut.XXXXXX")"
cleanup() { rm -rf "$TMP"; }
trap cleanup EXIT
...
echo "======================================================================"
echo "§1.1 paired-mutation meta-test for CM-COVENANT-114-162-PROPAGATION"
echo "fixtures under: $TMP"
echo "======================================================================"

# --- MUTATED fleet: a carrier MISSING the 11.4.162 literal ---
MUT_FLEET="$TMP/prop_mut"
mkdir -p "$MUT_FLEET/sub"
# good carrier (carries the anchor)
printf '## §11.4.162 OpenDesign UI design system mandate\nanchor 11.4.162 present here\n' > "$MUT_FLEET/AGENTS.md"
...
for c in CLAUDE.md AGENTS.md QWEN.md GEMINI.md; do
    printf '# carrier %s\n§11.4.162 — OpenDesign UI design system mandate anchor literal 11.4.162\n' "$c" \
        > "$CLEAN_FLEET/$c"
done
...
echo "======================================================================"
if [ "$rc" -eq 0 ]; then
    echo "✅ META PASS: CM-COVENANT-114-162-PROPAGATION is a genuine (non-bluff) gate"
else
    echo "❌ META FAIL: CM-COVENANT-114-162-PROPAGATION failed the §1.1 discriminator"
fi
echo "======================================================================"
exit "$rc"
