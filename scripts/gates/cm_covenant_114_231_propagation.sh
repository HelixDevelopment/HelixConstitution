#!/usr/bin/env bash
# cm_covenant_114_231_propagation.sh — CM-COVENANT-114-231-PROPAGATION gate
# (nano-precision model-tier selection: lightest-capable-model-first with
# dynamic mid-task escalation).
#
# ── Purpose ──────────────────────────────────────────────────────────────────
# §11.4.231 requires the anchor BLOCK (the anchor's own governance paragraph,
# not merely the bare literal `11.4.231` — §11.4.227(B)) to be present
...
#     the anchor (e.g. "... per §11.4.231(A) ...") is a CARRIER and MUST NOT
...
#                              for §11.4.185 and later anchors, incl. 231-233)
...
# so a numeric prefix-match (e.g. "2311") can never be mistaken for "231".
#
...
#
# ── Usage ────────────────────────────────────────────────────────────────────
#   cm_covenant_114_231_propagation.sh [--root <consumer-root>] [--quiet]
...
#
# ── Cross-references ──────────────────────────────────────────────────────────
#   §11.4.231 (nano-precision model-tier selection), §11.4.227(B) (anchor-block
...
#
# ── Exit codes ───────────────────────────────────────────────────────────────
#   0 — every discovered owned carrier carries exactly ONE `11.4.231` block,
...
# numeric prefix-match (e.g. never confuse "2311" with "231").
