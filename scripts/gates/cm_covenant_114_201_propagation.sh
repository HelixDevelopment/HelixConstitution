#!/usr/bin/env bash
# cm_covenant_114_201_propagation.sh — CM-COVENANT-114-201-PROPAGATION gate (every guard/gate MUST assert the REAL condition (a false-positive refusal is a FAIL-bluff)).
#
# ── Purpose ──────────────────────────────────────────────────────────────────
# §11.4.201 (every guard/gate MUST assert the REAL condition (a false-positive refusal is a FAIL-bluff)) requires
# the anchor literal `11.4.201` to be present in every owned governance
...
#
# ── Cross-references ──────────────────────────────────────────────────────────
#   §11.4.201 (guard-asserts-the-real-condition mandate), §11.4.157
#   (five-carrier lockstep), §11.4.35 (inheritance), §11.4.28 (decoupling),
#   §11.4.1 / §11.4.180 / §11.4.201(D) (the FAIL-bluff + liveness-proven + resource-detection-by-real-identity anchors §11.4.201 generalises to EVERY guard), §1.1 (paired mutation — strip
...
#
# ── Exit codes ───────────────────────────────────────────────────────────────
#   0 — every discovered owned carrier carries the `11.4.201` literal.
...
if [ "$fail" -gt 0 ]; then
    echo "❌ ${GATE}: FAIL — ${fail} owned carrier(s) missing §11.4.201 anchor"
    exit 1
fi
echo "✅ ${GATE}: PASS — every owned carrier carries §11.4.201"
exit 0
