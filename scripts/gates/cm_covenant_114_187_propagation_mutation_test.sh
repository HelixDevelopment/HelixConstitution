#!/usr/bin/env bash
# cm_covenant_114_187_propagation_mutation_test.sh — §1.1 paired-mutation
# meta-test for CM-COVENANT-114-187-PROPAGATION (§11.4.187).
#
# ── Purpose ──────────────────────────────────────────────────────────────────
# Proves CM-COVENANT-114-187-PROPAGATION is NOT a bluff gate: builds two
# disposable fleet fixtures under a temp dir — one with a carrier MISSING the
# `11.4.187` literal (MUST FAIL) and one where every carrier carries it (MUST
...
#
# ── Usage ────────────────────────────────────────────────────────────────────
#   cm_covenant_114_187_propagation_mutation_test.sh
#
# ── Side-effects ─────────────────────────────────────────────────────────────
...
#
# ── Dependencies ─────────────────────────────────────────────────────────────
#   bash, the sibling cm_covenant_114_187_propagation.sh gate script. Parses
#   clean under bash -n.
#
# ── Cross-references ──────────────────────────────────────────────────────────
#   §1.1 (paired mutation), §11.4.187, §11.4.157 (five-carrier lockstep),
...
GATE="${SCRIPT_DIR}/cm_covenant_114_187_propagation.sh"

[ -f "$GATE" ] || { echo "META: gate script missing: $GATE" >&2; exit 2; }

TMP="$(mktemp -d "${TMPDIR:-/tmp}/cm187_mut.XXXXXX")"
cleanup() { rm -rf "$TMP"; }
trap cleanup EXIT
...
echo "======================================================================"
echo "§1.1 paired-mutation meta-test for CM-COVENANT-114-187-PROPAGATION"
echo "fixtures under: $TMP"
echo "======================================================================"

# --- MUTATED fleet: a carrier MISSING the 11.4.187 literal ---
MUT_FLEET="$TMP/prop_mut"
mkdir -p "$MUT_FLEET/sub"
# good carrier (carries the anchor)
printf '## §11.4.187 automatic multi-track ruler orchestration\nanchor 11.4.187 present here\n' > "$MUT_FLEET/AGENTS.md"
...
for c in CLAUDE.md AGENTS.md QWEN.md GEMINI.md; do
    printf '# carrier %s\n§11.4.187 — automatic multi-track ruler orchestration anchor literal 11.4.187\n' "$c" \
        > "$CLEAN_FLEET/$c"
done
...
echo "======================================================================"
if [ "$rc" -eq 0 ]; then
    echo "✅ META PASS: CM-COVENANT-114-187-PROPAGATION is a genuine (non-bluff) gate"
else
    echo "❌ META FAIL: CM-COVENANT-114-187-PROPAGATION failed the §1.1 discriminator"
fi
echo "======================================================================"
exit "$rc"
