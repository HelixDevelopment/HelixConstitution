#!/usr/bin/env bash
# cm_covenant_114_233_propagation.sh — CM-COVENANT-114-233-PROPAGATION gate
# (standing anti-mess control plane: level-triggered idempotent invariant-
# catalogue reconciliation loop over the WHOLE persistent System state).
#
# ── Purpose ──────────────────────────────────────────────────────────────────
# §11.4.233 requires the anchor BLOCK (the anchor's own governance paragraph,
# not merely the bare literal `11.4.233` — §11.4.227(B)) to be present
...
#     the anchor (e.g. "... per §11.4.233(A) ...") is a CARRIER and MUST NOT
...
#                              for §11.4.185 and later anchors, incl. 233-233)
...
# so a numeric prefix-match (e.g. "2331") can never be mistaken for "233".
#
...
#
# ── Usage ────────────────────────────────────────────────────────────────────
#   cm_covenant_114_233_propagation.sh [--root <consumer-root>] [--quiet]
...
#
# ── Cross-references ──────────────────────────────────────────────────────────
#   §11.4.233 (standing anti-mess control plane), §11.4.227(B) (anchor-block
...
#
# ── Exit codes ───────────────────────────────────────────────────────────────
#   0 — every discovered owned carrier carries exactly ONE `11.4.233` block,
...
ANCHOR="11.4.233"
GATE="CM-COVENANT-114-233-PROPAGATION"
...
# numeric prefix-match (e.g. never confuse "2331" with "233").
