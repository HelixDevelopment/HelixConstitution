...
`Implemented (→ Fixed.md)` is a §11.4.25 violation. Pre-build gate
...
A `Reopened` entry without `**Reopened-Details:**` is a §11.4.26
...
per §11.4.27" / "Demoted from constitution to <project> per
§11.4.27" line in the commit message.
...
Composes with §11.4.17 (universal-vs-project classification — §11.4.27
