...
> "Every release tag and version name we create — on the main repository and on every Submodule we own — MUST be prefixed with the project's release prefix, e.g. `helix_translate-1.0.0-dev-0.0.1`. The prefix MUST come from `HELIX_RELEASE_PREFIX` in our `.env` if it is set, otherwise from the lowercased project root directory name. The SAME prefix MUST be used across the main repo and all owned Submodules in one release so a release is greppable across every repository."

Every release tag AND every version name created on the main repository AND on every owned-by-us submodule (per §11.4.28) MUST be prefixed with the project's release prefix, in the form `<PREFIX>-<version>` (canonical example: `helix_translate-1.0.0-dev-0.0.1`). The prefix makes a release identifiable + greppable across every repository the release spans, so a single `git tag -l '<PREFIX>-*'` (or equivalent) enumerates the whole release surface in one query.
...
**§11.4.166 — Universal Semgrep static analysis mandate (User mandate, 2026-06-21).** Verbatim operator mandate: "semgrep MUST be available and used by all projects governed by this Constitution for static analysis / security scanning of all code changes, before any commit or push to any upstream! semgrep MUST be a MANDATORY tool installed on every development host, auto-configured, built if needed, and available on the system PATH for ALL users via .bashrc/.zshrc integration."

Every consuming project under this Constitution MUST ensure Semgrep (https://github.com/semgrep/semgrep) is installed, configured, and available on the system PATH for every user. (1) **Installation** — `pip install --user semgrep` is the fast path. The constitution submodule provides `scripts/semgrep/semgrep_setup.sh` (auto-detects pip or builds from submodule). (2) **PATH integration** — source `scripts/semgrep/semgrep_path.sh` from `.bashrc` / `.zshrc` to ensure semgrep is on every user's PATH. A post-clone/post-pull hook MUST auto-source this. (3) **Scan before every commit+push** — every commit wrapper AND pre-push hook MUST run `semgrep scan --config auto --error` on the staged/committed diff and BLOCK the commit/push (non-zero exit) on any finding. The pre-commit hook at `scripts/hooks/pre-commit` is the canonical integration point. (4) **MCP integration** — the Semgrep MCP (`semgrep_scan`, `semgrep_findings`, `semgrep_scan_with_custom_rule` tools) MUST be connected to every CLI agent used by developers per the same agent-wiring pattern as CodeGraph (§11.4.78 step 3). (5) **Anti-bluff validation** — `scripts/semgrep/semgrep_validate.sh` verifies semgrep is installed, functional, registry-reachable, and the pre-commit hook fires, all with captured evidence per §11.4.69. (6) **docs_chain** — a `.docs_chain/contexts/semgrep_status.yaml` context registers the semgrep integration for docs_chain auto-sync. (7) **Constitution as single source** — semgrep submodule pointer + build/install scripts + PATH integration + hook wiring all live in the constitution submodule, inherited by reference per §11.4.28/§11.4.35. Consuming projects invoke at `$CONST_DIR/scripts/semgrep/*.sh`, never copy.

Classification: universal (§11.4.17). Propagation gate `CM-COVENANT-114-166-PROPAGATION` + recommended gate `CM-SEMGREP-WIRED` (semgrep on PATH + pre-commit hook fires + scan blocks on real findings). Paired §1.1 meta-test mutation.

Composes §11.4.78/.79/.80/.10/.27/.28/.30/.35/.65/.69/.77/.106/.109/.125/.142/.
