...
**§11.4.166 — Universal Semgrep static analysis mandate (User mandate, 2026-06-21).** Verbatim: "semgrep MUST be available and used by all projects governed by this Constitution for static analysis / security scanning of all code changes, before any commit or push to any upstream! semgrep MUST be installed on every development host, auto-configured, and available on the system PATH for ALL users via .bashrc/.zshrc integration." Every consuming project MUST ensure semgrep is installed + configured: (1) `pip install --user semgrep` or via `scripts/semgrep/semgrep_setup.sh`; (2) PATH integration via sourcing `scripts/semgrep/semgrep_path.sh` from .bashrc/.zshrc; (3) scan before every commit+push via `semgrep scan --config auto --error` — blocks on any finding; (4) MCP integration per §11.4.78 step 3 (same pattern as CodeGraph); (5) anti-bluff validation via `scripts/semgrep/semgrep_validate.sh` per §11.4.69; (6) docs_chain context at `.docs_chain/contexts/semgrep_status.yaml`; (7) all scripts in constitution submodule, inherited by reference per §11.4.28/§11.4.35. Classification: universal (§11.4.17). Propagation gate `CM-COVENANT-114-166-PROPAGATION` + recommended gate `CM-SEMGREP-WIRED` + paired §1.1 meta-test mutation. Composes §11.4.78/.79/.80/.10/.27/.28/.30/.35/.65/.69/.77/.106/.109/.125/.142/.
